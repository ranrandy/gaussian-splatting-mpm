...
#         torch.tensor([1, 1, 1]).reshape((1, 3)).cuda()
#     )
...
def simulate(model_args : ModelParams, sim_args : MPMParams, render_args : RenderParams):
    ...
    # # Test for adding a surface collider
    mpm_solver.add_surface_collider(tm.vec3(0, 0, 0.5), tm.vec3(0, 0, 1))
    mpm_solver.add_surface_collider(tm.vec3(0, 0.5, 0), tm.vec3(0, 1, 0))
